...
(defn render-election [m]
  (let [{:keys [description polling-place-url]} m]
    [:a {:href polling-place-url}
     description]))

(defn results [request]
  (let [elections (get-in request [:results :elections])
        election-count (get-in request [:results :count])]
    (into [:div [:h2 (str "Search Results: " election-count " elections")]]
          (mapv render-election elections))))
